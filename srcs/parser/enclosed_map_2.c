...
/*   Updated: 2020/07/12 02:13:57 by jlensing      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int		**fill_map_check(t_data *data, int **tempmap, int i, int j)
{
	...
	if (i > 0 && j > 0 && tempmap[i - 1][j - 1] == 2)
		tempmap[i][j] = 2;
	if (i < data->map_info->mapheight - 1 && j > 0 &&
		tempmap[i + 1][j - 1] == 2)
		tempmap[i][j] = 2;
	if (j < data->map_info->mapwidth - 1 && i > 0 &&
		tempmap[i - 1][j + 1] == 2)
		tempmap[i][j] = 2;
	if (j < data->map_info->mapwidth - 1 && i < data->map_info->mapheight - 1 &&
		tempmap[i + 1][j + 1] == 2)
		tempmap[i][j] = 2;
	return (tempmap);
}
