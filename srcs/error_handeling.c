...
/*   Updated: 2020/07/12 01:23:46 by jlensing      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (data->error == INVALID_MAP_FILE || data->error == MAP_SURROUND)
	...
	if (data->error == MAP_LOADING)
		ft_printf("Error\nLoading map failed\n");
	if (data->error == ALLOC_FAILURE || data->error == MAP_ALLOC_FAILURE)
	...
	if (data->error == WRITE_FAILED)
		ft_printf("Error\nWriting to BMP file failed\n");
	if (data->error == TEXTURE_CORRUPT)
		ft_printf("Error\nTexture file is corrupt\n");
	exit_game(data);
}
